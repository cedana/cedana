...
// By default, implements only the process freeze subcommand.
// Other subcommands, for e.g. runc, are imported from installed plugins, as they could
// declare their own flags and subcommands. For runc, see plugins/runc/cmd/freeze.go.
...
func init() {
	...
	features.FreezeCmd.IfAvailable(
		func(name string, pluginCmd *cobra.Command) error {
			freezeCmd.AddCommand(pluginCmd)

			// Apply all the flags from the plugin command to job subcommand (as optional flags),
			// since the job subcommand can be used to freeze any managed entity (even from plugins, like runc),
...
var jobFreezeCmd = &cobra.Command{
	Use:               "job <JID>",
	Short:             "Freeze a managed process/container (job)",
	Args:              cobra.ExactArgs(1),
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if jobType != "process" {
			err = features.FreezeCmd.IfAvailable(
				func(name string, pluginCmd *cobra.Command) error {
