...
func (s *service) CRIOImagePush(ctx context.Context, args *task.CRIOImagePushArgs) (resp *task.CRIOImagePushResp, err error) {
	...
	if err := crio.RootfsMerge(ctx, args.OriginalImageRef, args.NewImageRef, args.RootfsDiffPath, args.ContainerStorage, args.PullRegistryAuthToken); err != nil {
		return resp, err
	}

	s.logger.Debug().Msgf("CRIO image push started with new image: %s", args.NewImageRef)
	if err := crio.ImagePush(ctx, args.NewImageRef, args.PushRegistryAuthToken); err != nil {
		return resp, err
	}
