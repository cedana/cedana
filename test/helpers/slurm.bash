...
PY
        ...
        if [ "${GPU:-0}" = "1" ]; then
            ...
            docker exec "$c" sh -c 'echo "=== nvidia driver/cuda ==="; nvidia-smi --query-gpu=name,driver_version,memory.total --format=csv 2>&1 || true; echo "=== dmesg (segfault/oom/nvidia/cedana) ==="; dmesg -T 2>/dev/null | grep -iE "segfault|cedana-gpu|nvidia|nvrm|out of memory|killed process|general protection|traps:" | tail -80 || true; echo "=== core_pattern ==="; cat /proc/sys/kernel/core_pattern 2>/dev/null || true; echo "=== core dumps ==="; ls -la /tmp/core* /var/lib/systemd/coredump/* /data/core* core* 2>/dev/null || true; echo "=== cedana-gpu controller logs ==="; for f in /var/log/cedana-gpu*.log /var/log/cedana/gpu*.log /tmp/cedana-gpu*.log; do [ -f "$f" ] && { echo "--- $f ---"; tail -200 "$f"; }; done 2>/dev/null || true' >"$cdir/gpu-controller-debug.txt" 2>&1 || true
        fi
