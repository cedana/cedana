...
@test "[$GPU_INFO] run GPU container (GPU binary, without daemon)" {
    ...
    debug cedana run crun --bundle "$bundle" --jid "$jid" --gpu-enabled --no-server
}
...
@test "[$GPU_INFO] dump GPU container (non-GPU binary)" {
    ...
    cedana run crun --bundle "$bundle" --jid "$jid" --gpu-enabled
    watch_logs "$jid"
...
@test "[$GPU_INFO] restore GPU container (mem throughput saxpy, without daemon)" {
    ...
    debug cedana restore crun --path "$dump_file" --id "$jid" --bundle "$bundle" --detach --no-server
