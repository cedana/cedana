...
type Manager interface {
	...
	Attach(ctx context.Context, lifetime context.Context, jid string, user *syscall.Credential, pid <-chan uint32) error

	// AttachAsync calls Attach in background.
	// Returns a channel that will receive an error if the attach fails.
	AttachAsync(ctx context.Context, lifetime context.Context, jid string, user *syscall.Credential, pid <-chan uint32) <-chan error
...
func (ManagerMissing) Attach(ctx context.Context, lifetime context.Context, jid string, user *syscall.Credential, pid <-chan uint32) error {
	return fmt.Errorf("GPU manager missing")
}

func (ManagerMissing) AttachAsync(ctx context.Context, lifetime context.Context, jid string, user *syscall.Credential, pid <-chan uint32) <-chan error {
	err := make(chan error)
