...
func NewNotifySocket(root, notifySocketHost string, id string) *notifySocket {
	if notifySocketHost == "" {
		return nil
	}
...
func NotifySocketStart(root, notifySocketHost, id string) (*notifySocket, error) {
	notifySocket := NewNotifySocket(root, notifySocketHost, id)
	if notifySocket == nil {
		return nil, nil
	}
