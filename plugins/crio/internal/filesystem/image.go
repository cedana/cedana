...
func parseImageNames(image *storage.Image) (someName *RegistryImageReference, tags []reference.NamedTagged, digests []reference.Canonical, err error) {
	...
	if len(digests) > 0 {
		best := RegistryImageReferenceFromRaw(digests[0])
		someName = &best
	}
	if len(tags) > 0 {
		best := RegistryImageReferenceFromRaw(tags[0])
		someName = &best
	}
...
func moveSelfToCgroup(cgroup string) error {
	...
	return RunUnderSystemdScope(NewDbusConnManager(unshare.IsRootless()), os.Getpid(), slice, unitName)
}
...
func pullImageChild() {
	...
	imageName, err := ParseRegistryImageReferenceFromOutOfProcessData(args.ImageName)
	if err != nil {
		fmt.Fprintf(os.Stderr, "%v", err)
		os.Exit(1)
	}
...
func (svc *imageService) CandidatesForPotentiallyShortImageName(systemContext *types.SystemContext, imageName string) ([]RegistryImageReference, error) {
	...
	for i := range resolved.PullCandidates {
		...
		images[i] = RegistryImageReferenceFromRaw(ref)
	}
