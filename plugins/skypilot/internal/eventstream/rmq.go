...
func New(ctx context.Context, cedana *client.Client, propagator *cedanagosdk.ApiClient) (*EventStream, error) {
	...
	clientName := fmt.Sprintf("cedana-skypilot-%s-%d", hostname, time.Now().UnixNano())
...
func (es *EventStream) StartCheckpointsConsumer(ctx context.Context) error {
	...
	queueName := "cedana_skypilot_helper-" + rand.Text()
	consumer, err := rabbitmq.NewConsumer(
		conn,
		queueName,
		rabbitmq.WithConsumerOptionsExchangeName("skypilot_broadcast_request"),
		rabbitmq.WithConsumerOptionsConcurrency(10),
		rabbitmq.WithConsumerOptionsExchangeDeclare,
		rabbitmq.WithConsumerOptionsExchangeKind("fanout"),
		rabbitmq.WithConsumerOptionsConsumerName("cedana_skypilot_helper"),
...
func (es *EventStream) checkpointHandler(ctx context.Context) rabbitmq.Handler {
	return func(msg rabbitmq.Delivery) rabbitmq.Action {
		log.Trace().Msgf("received skypilot checkpoint request: %s", string(msg.Body))
...
func (es *EventStream) publishCheckpoint(
	...
	err = publisher.Publish(data, []string{"skypilot_checkpoint_response"})
	if err != nil {
		return err
	}
	if dumpErr != nil {
		log.Error().Err(dumpErr).Msg("skypilot checkpoint published with error")
	} else {
		log.Info().Str("path", path).Bool("GPU", ci.GPU).Msg("skypilot checkpoint published")
	}
	return nil
}
